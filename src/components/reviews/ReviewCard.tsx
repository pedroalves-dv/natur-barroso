...
function StarIcon({ filled }: { filled: boolean }) {
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill={filled ? '#C8882A' : 'none'} stroke="#C8882A" strokeWidth="1.5" aria-hidden="true">
...
export default function ReviewCard({ review }: Props) {
  return (
    <article className="bg-white rounded-xl p-6 shadow-sm flex flex-col gap-4">
      ...
      <p className="text-granite/70 text-sm leading-relaxed flex-1">"{review.text}"</p>
      ...
      <div className="flex items-end justify-between gap-2 pt-2 border-t border-granite/8">
        <div>
          ...
          <p className="text-granite/50 text-xs">{review.country} · {review.tour}</p>
        </div>
        <div className="flex flex-col items-end gap-1">
          <span className="text-xs text-granite/40">{review.date}</span>
          <span className="text-xs font-medium text-amber">{SOURCE_LABELS[review.source]}</span>
        </div>
      </div>
    </article>
  );
