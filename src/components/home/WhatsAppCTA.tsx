import { getTranslations } from "next-intl/server";

const CONTACT_NUMBER = "351960000000";

export default async function WhatsAppCTA() {
  const t = await getTranslations("HomePage");

  const waHref = `https://wa.me/${CONTACT_NUMBER}?text=${encodeURIComponent("Olá! Tenho interesse num tour da Natur Barroso.")}`;
  const telHref = `tel:+${CONTACT_NUMBER}`;

  return (
    <section className="py-24 bg-fog">
      <div className="max-w-7xl mx-auto px-4 text-center">
        {/* Custom tours badge */}
        <div className="inline-flex items-center gap-1.5 bg-amber/10 text-amber text-xs font-medium tracking-wide uppercase px-3 py-1 rounded-full mb-6">
          <svg
            width="12"
            height="12"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
            aria-hidden="true"
          >
            <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
          </svg>
          {t("whatsappCtaCustomLabel")}
        </div>

        {/* WhatsApp icon */}
        <div className="flex justify-center mb-6">
          <div
            className="inline-flex items-center justify-center w-14 h-14 rounded-full"
            style={{ backgroundColor: "#25D36622" }}
          >
            <svg
              width="28"
              height="28"
              viewBox="0 0 24 24"
              fill="#25D366"
              aria-hidden="true"
            >
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z" />
              <path d="M12 0C5.373 0 0 5.373 0 12c0 2.126.554 4.118 1.524 5.847L.057 23.786c-.078.31.196.584.506.506l5.939-1.467A11.946 11.946 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.878 0-3.636-.51-5.145-1.4l-.369-.217-3.527.871.885-3.427-.241-.386A9.94 9.94 0 0 1 2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z" />
            </svg>
          </div>
        </div>

        <h2 className="text-5xl md:text-6xl font-serif leading-tight text-granite mb-4 tracking-[-0.01em]">
          {t("whatsappCtaTitle")}
        </h2>
        <p className="text-granite/60 leading-relaxed mb-10 max-w-md mx-auto">
          {t("whatsappCtaBody")}
        </p>

        {/* Dual CTAs */}
        <div className="flex flex-col sm:flex-row gap-3 items-center justify-center">
          <a
            href={waHref}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-3 px-8 py-3.5 rounded-full font-medium text-white transition-transform hover:scale-[1.02] active:scale-[0.98]"
            style={{ backgroundColor: "#25D366" }}
          >
            <svg
              width="20"
              height="20"
              viewBox="0 0 24 24"
              fill="currentColor"
              aria-hidden="true"
            >
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z" />
              <path d="M12 0C5.373 0 0 5.373 0 12c0 2.126.554 4.118 1.524 5.847L.057 23.786c-.078.31.196.584.506.506l5.939-1.467A11.946 11.946 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.878 0-3.636-.51-5.145-1.4l-.369-.217-3.527.871.885-3.427-.241-.386A9.94 9.94 0 0 1 2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z" />
            </svg>
            {t("whatsappCtaButton")}
          </a>

          <a
            href={telHref}
            className="inline-flex items-center gap-2.5 px-8 py-3.5 rounded-full font-medium text-granite border border-granite/25 transition-colors hover:bg-granite/5 active:scale-[0.98]"
          >
            <svg
              width="18"
              height="18"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="1.5"
              strokeLinecap="round"
              strokeLinejoin="round"
              aria-hidden="true"
            >
              <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z" />
            </svg>
            {t("whatsappCtaCallButton")}
          </a>
        </div>
      </div>
    </section>
  );
}
