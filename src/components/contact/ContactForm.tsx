...
export default function ContactForm({ locale, labels }: Props) {
  ...
  const subjectOptions = locale === 'pt' ? SUBJECT_OPTIONS_PT : SUBJECT_OPTIONS_EN;
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center gap-4">
        <div className="w-12 h-12 rounded-full bg-forest/10 flex items-center justify-center">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#2D4A2F" strokeWidth="2.5">
  ...
  return (
    <div>
      <h2 className="font-serif text-xl text-granite mb-1">{labels.title}</h2>
      <p className="text-sm text-granite/60 mb-6">{labels.subtitle}</p>

      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold uppercase tracking-wider text-granite/50">
              {labels.nameLabel} *
            </label>
            <input
              name="name"
        ...
        <button
          ...
          className="px-8 py-3 bg-forest text-white rounded-full text-sm font-medium hover:bg-moss transition-colors disabled:opacity-60 self-start"
        >
