      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-12">
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="flex-1 flex flex-col gap-14">
            ...
            {/* Overview */}
            <section>
              <h2 className="text-2xl font-serif text-granite mb-5">{t('overview')}</h2>
            ...
            {/* Highlights */}
            <section>
              <h2 className="text-2xl font-serif text-granite mb-5">{t('highlights')}</h2>
              <ul className="grid sm:grid-cols-2 gap-3">
                {tour.highlights.map((item) => (
                  <li key={item} className="flex items-start gap-2.5 text-sm text-granite/80">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#C8882A" strokeWidth="2.5" className="shrink-0 mt-0.5" aria-hidden="true">
            ...
            <section>
              <h2 className="text-2xl font-serif text-granite mb-6">{t('inclusions')}</h2>
              <TourInclusions
            ...
            {/* Pricing */}
            <section>
              <TourPricingTable pricing={tour.pricing} title={t('pricing')} locale={locale} />
            </section>

            {/* Meet your guide */}
            <section>
              <h2 className="text-2xl font-serif text-granite mb-6">{t('meetGuide')}</h2>
              <div className="flex items-start gap-5 p-6 bg-white rounded-xl border border-fog">
                ...
                />
                <div>
                  <p className="font-semibold text-granite mb-0.5">{tour.guide.name}</p>
                  <p className="text-xs text-granite/50 mb-3">{tour.guide.languages.join(' · ')}</p>
                  <p className="text-sm text-granite/70 leading-relaxed mb-3">{tour.guide.bio}</p>
                  <div className="flex flex-wrap gap-1.5">
                    {tour.guide.specialties.map((s) => (
                      <span key={s} className="text-xs px-2 py-0.5 bg-fog rounded-full text-granite/60">
                        {s}
                      </span>
                    ))}
                  </div>
