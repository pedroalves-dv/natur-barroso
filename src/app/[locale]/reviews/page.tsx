    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 text-center">
          ...
          <div className="flex items-center justify-center gap-2 mb-3">
            {Array.from({ length: 5 }).map((_, i) => (
              <svg key={i} width="24" height="24" viewBox="0 0 24 24" fill="#C8882A" aria-hidden="true">
          ...
          {featuredReviews.map((review) => (
            ...
            >
              <div className="flex gap-0.5">
                {Array.from({ length: 5 }).map((_, i) => (
                  <svg key={i} width="14" height="14" viewBox="0 0 24 24" fill="#C8882A" aria-hidden="true">
                    <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                  </svg>
                ))}
              </div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-xl mx-auto px-4 md:px-6 text-center">
          <h2 className="font-serif text-2xl text-granite mb-4">{t('leaveReviewTitle')}</h2>
          ...
          <p className="mt-4 text-xs text-granite/40">
            {isPt ? 'Obrigado pela sua avaliação — faz uma diferença enorme.' : 'Thank you for reviewing — it makes a huge difference.'}
          </p>
        </div>
      </section>
    </>
